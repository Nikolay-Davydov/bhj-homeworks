...
const productsQuantityControls = document.querySelectorAll('.product__quantity-controls');

for (let i = 0; i < productsAdd.length; i++) {
    productsAdd[i].addEventListener('click', addToCart);
}

for (let i = 0; i < productsQuantityControls.length; i++) {
    productsQuantityControls[i].addEventListener('click', changeQuantity);
}
